...
type Battle struct {
	...
	// 本回合双方各自“刚刚受到的一次伤害”和“累计伤害”（为兼容部分技能预留）
	PlayerDamageTakenThisTurn int
	EnemyDamageTakenThisTurn  int
	PlayerLastDamageTaken     int
	EnemyLastDamageTaken      int
	
	// 本回合开始时双方的 HP（用于部分基于“本回合受到伤害”的技能）
	PlayerHpAtTurnStart int
	EnemyHpAtTurnStart  int
	
	// 本回合内“对方上一次通过技能造成的伤害”（不含状态伤害），用于克制这类“将本次所受伤害 n 倍反击”的技能
	PlayerDamageFromEnemySkillThisTurn int
	EnemyDamageFromPlayerSkillThisTurn int
}
...
func GetTypeMultiplier(atkType, defType int) float64 {
	return typechart.GetInstance().Multiplier(atkType, defType)
}

// GetTypeMultiplierDual 处理双属性防守：倍率 = 两属性倍率相乘。
// 注意：具体的“多少倍”完全由配置矩阵决定，这里不再额外对“攻击方双属性”做加成。
...
// 兼容旧接口：现在已经不再根据攻击方双属性做额外倍率，同时保留函数签名供 handlers 调用。
// 具体克制关系仍然只依赖技能 Type 与防守方 Type/Type2。
func GetTypeMultiplierWithAttackerDual(skillType, attackerType, attackerType2, defType1, defType2 int) float64 {
	return GetTypeMultiplierDual(skillType, defType1, defType2)
}
...
// parseMultiHitWithExtraFixed 解析“连续攻击+附加造成伤害百分比固定伤害”类技能参数。
// 这里按照原技能描述，**专门**为「牙连绝袭击」实现：
// - 固伤百分比固定为 10%
// - 攻击次数范围固定为 5~8 次
// 这样可以避免 SideEffect / SideEffectArg 在不同数据源下不一致导致逻辑失效。
func parseMultiHitWithExtraFixed(skill *skills.Skill) (enabled bool, extraPercent int, minHits int, maxHits int) {
	if skill == nil {
		return false, 0, 0, 0
	}
	
	// 通过技能 ID 或名称识别「牙连绝袭击」
	if skill.ID == 10375 || skill.Name == "牙连绝袭击" {
		return true, 10, 5, 8
	}
...
func (b *Battle) ExecuteTurn(playerSkillID int) *BattleLog {
	...
	// 重置本回合伤害统计
	b.PlayerDamageTakenThisTurn = 0
	b.EnemyDamageTakenThisTurn = 0
	b.PlayerLastDamageTaken = 0
	b.EnemyLastDamageTaken = 0
	b.PlayerDamageFromEnemySkillThisTurn = 0
	b.EnemyDamageFromPlayerSkillThisTurn = 0
	
	// 记录回合开始时双方 HP（在状态伤害结算之后）
	b.PlayerHpAtTurnStart = b.Player.HP
	b.EnemyHpAtTurnStart = b.Enemy.HP
	...
	if playerSkillID == 0 {
		// 敌方行动
		if enemyCanAct {
			result := ExecuteAttack(b, b.Enemy, b.Player, enemySkill, 0, true)
		...
		if playerFirst {
			// 玩家先攻
			if playerCanAct {
				result := ExecuteAttack(b, b.Player, b.Enemy, playerSkill, b.UserID, true)
			...
			if b.Enemy.HP <= 0 {
				...
				// 敌方反击
				if enemyCanAct {
					result := ExecuteAttack(b, b.Enemy, b.Player, enemySkill, 0, false)
					b.Player.HP = result.TargetRemainHp
			...
			// 玩家反击
			if playerCanAct {
				result := ExecuteAttack(b, b.Player, b.Enemy, playerSkill, b.UserID, false)
				b.Enemy.HP = result.TargetRemainHp
				log.SecondAttack = result
			} else {
				log.SecondAttack = &AttackResult{
...
// ExecuteAttack 执行攻击
func ExecuteAttack(b *Battle, attacker, defender *Pet, skill *skills.Skill, attackerUserID int64, isFirst bool) *AttackResult {
	result := &AttackResult{
	...
	// 特殊：处理“威力×(n~m) 一次攻击 + 附加造成伤害X%的固定伤害”类型技能
	if enabled, extraPercent, minHits, maxHits := parseMultiHitWithExtraFixed(skill); enabled {
		hitCountRange := maxHits - minHits + 1
		if hitCountRange <= 0 {
			hitCountRange = 1
		}
		hits := minHits + rand.Intn(hitCountRange)
		if hits <= 0 {
			hits = 1
		}
		
		// 正确逻辑：本回合只出招一次，但是内部连续攻击多次，
		// 先把每一击按威力25分别结算，总和为“多次出招伤害”，
		// 然后在这个总伤害基础上再附加 X% 的固定伤害。
		if hits < 1 {
			hits = 1
		}
		
		totalBase := 0
		var finalTypeMod float64 = 1.0
		anyCrit := false
		
		for i := 0; i < hits; i++ {
			perCrit := CheckCrit(attacker, defender, skill, isFirst)
			perDmg, typeMod, _ := CalculateDamage(attacker, defender, skill, perCrit)
			if perDmg < 0 {
				perDmg = 0
			}
			totalBase += perDmg
			finalTypeMod = typeMod
			if perCrit {
				anyCrit = true
			}
		}
		
		// 在“多次出招总伤害”基础上附加 X% 固定伤害（只加一次）
		extra := totalBase * extraPercent / 100
		if extra < 0 {
			extra = 0
		}
		
		totalDamage := totalBase + extra
		
		defender.HP = max(0, defender.HP-totalDamage)
		
		result.Damage = totalDamage
		result.IsCrit = anyCrit
		result.TypeMod = finalTypeMod
	...
	// 特殊：克制（EffectID=34）
	// 规则（按你说的）：
	// - 必须“后出手”时使用才生效；若本回合先手使用则伤害=0；
	// - 只记录“对方本回合上一次技能打在自己身上的伤害值”（不含状态伤害）；
	// - 命中后直接对对方造成 该伤害值×n（这里 n=2）的反击伤害。
	if skill.EffectID == 34 {
		// 若是先手出招，克制不生效
		if isFirst {
			damage = 0
		} else {
		mult := 2
		if skill.SideEffectArg != "" {
			args := strings.Fields(skill.SideEffectArg)
			if len(args) >= 1 {
			...
			var last int
			if attacker == b.Player {
				// 我方后出手，用“敌方本回合上一击对我造成的伤害”
				last = b.PlayerDamageFromEnemySkillThisTurn
			} else if attacker == b.Enemy {
				// 敌方后出手，用“我方本回合上一击对敌造成的伤害”
				last = b.EnemyDamageFromPlayerSkillThisTurn
			}
	...
	// 记录本回合受到的总伤害，供“克制”等技能使用
	if defender == b.Player {
		b.PlayerDamageTakenThisTurn += damage
		b.PlayerLastDamageTaken = damage
		// 记录“敌方这一次技能对玩家造成的伤害”
		if attacker == b.Enemy {
			b.PlayerDamageFromEnemySkillThisTurn = damage
		}
	} else if defender == b.Enemy {
		b.EnemyDamageTakenThisTurn += damage
		b.EnemyLastDamageTaken = damage
		// 记录“玩家这一次技能对敌方造成的伤害”
		if attacker == b.Player {
			b.EnemyDamageFromPlayerSkillThisTurn = damage
		}
	}
